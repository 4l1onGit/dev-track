...
const RecentNotesPanel = async ({ notesPage }: { notesPage: number }) => {
  const { notes, total } = await getUserNotes(notesPage);

  return (
    <HomePanel className="h-2/3 flex flex-col space-y-4">
      <div className="flex justify-between">
        <h2 className="text-2xl text-gray-300 px-1">
          Recently Notes ({total})
        </h2>
